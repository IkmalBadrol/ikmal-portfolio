...
const Crosshair = ({ color = 'white', containerRef }: CrosshairProps) => {
  ...
  useEffect(() => {
    ...
    const onFirstMove = () => {
      ...
      gsap.to([lineHorizontalRef.current, lineVerticalRef.current], { duration: 0.9, ease: 'power3.out', opacity: 1 });
      rafId = requestAnimationFrame(render);
    ...
    }).to(primitiveValues, { duration: 0.5, ease: 'power1.out', startAt: { turbulence: 1 }, turbulence: 0 });

    const enter = () => tl.restart();
    const leave = () => { tl.progress(1); tl.kill(); };
