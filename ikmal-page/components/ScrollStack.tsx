...
export const ScrollStackItem: React.FC<ScrollStackItemProps> = ({ children, background, itemClassName = '' }) => (
  <div
    className={`scroll-stack-card relative w-full h-auto my-8 overflow-hidden rounded-[40px] box-border origin-top will-change-transform 
      /* --- Liquid Glass Classes --- */
      bg-zinc-900/30 bg-gradient-to-br from-white/10 to-transparent 
  ...
  >
    ...
    {/* Optional: Adds a subtle inner shine for a more realistic glass edge */}
    <div className="absolute inset-0 rounded-[40px] pointer-events-none ring-1 ring-inset ring-white/5 z-20" />
...
const ScrollStack: React.FC<ScrollStackProps> = ({
  ...
  const updateCardTransforms = useCallback(() => {
    ...
    cardsRef.current.forEach((card, i) => {
      ...
      const cardTop = cardsOffsetsRef.current[i] || 0;
      ...
      if (blurAmount) {
        ...
        for (let j = 0; j < cardsRef.current.length; j++) {
          const jCardTop = cardsOffsetsRef.current[j] || 0;
